...
const HOME_NAV = [
  { label: 'Home', href: '#home' }, { label: 'About', href: '#about' }, { label: 'Workspace', href: '#workspace' },
  { label: 'Skills', href: '#skills' }, { label: 'Journey', href: '#journey' }, { label: 'Projects', href: '#projects' },
  { label: 'Education', href: '#education' }, { label: 'Work With Me', href: '/freelance' }, { label: 'Contact', href: '#contact' },
];

const FREELANCE_NAV = [
  { label: 'Home', href: '/freelance' }, { label: 'Services', href: '#services' }, { label: 'Work', href: '#work' },
...
export default function Header() {
  ...
  const navItems = isFreelance ? FREELANCE_NAV : HOME_NAV;
  const [scrolled, setScrolled] = useState(false);
  const [activeSection, setActiveSection] = useState(isFreelance ? 'services' : 'home');
    ...
    <>
      <header className={`fixed top-0 z-50 w-full transition-all duration-500 ${scrolled ? 'py-3' : 'py-5'}`}>
        <div className={`mx-4 rounded-2xl transition-all duration-500 md:mx-8 ${scrolled ? 'glass border border-border/60' : ''}`}>
          <nav className="flex h-14 items-center justify-between px-5 md:px-8">
            <a href={isFreelance ? '/freelance' : '#home'} aria-label="Hariom Builds home" className="transition-opacity hover:opacity-80"><BrandMark compact /></a>

            <div className="hidden items-center gap-1 lg:flex">
              {navItems.map((item) => {
                const isActive = item.href.startsWith('#') ? activeSection === item.href.slice(1) : pathname === item.href;
                return <a key={item.label} href={item.href} className={`relative rounded-lg px-3 py-1.5 font-mono text-xs tracking-wider transition-all duration-300 ${isActive ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}>
            ...
            <div className="flex items-center gap-3">
              <a href={isFreelance ? '#contact' : '#contact'} className="hidden items-center gap-2 rounded-full bg-primary px-4 py-2 font-mono text-xs font-semibold tracking-wide text-primary-foreground transition-all duration-300 hover:shadow-lg md:flex">Let&apos;s Connect</a>
      ...
      <div className={`fixed inset-0 z-40 transition-all duration-500 lg:hidden ${menuOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`} style={{ background: 'rgba(10,11,14,0.96)', backdropFilter: 'blur(20px)' }}>
        <div className="flex h-full flex-col items-center justify-center gap-6">
          ...
          <a href="#contact" onClick={() => setMenuOpen(false)} className="mt-4 rounded-full bg-primary px-8 py-3 font-mono text-sm font-semibold tracking-wide text-primary-foreground">Let&apos;s Connect</a>
        </div>
      </div>
    </>
  );
