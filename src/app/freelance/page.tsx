...
const services = [
  { n: '01', title: 'Business Websites', text: 'Professional websites built around your services, your customers and the action you want visitors to take.', tags: ['Responsive UI', 'Clear CTA', 'Mobile-first'] },
  { n: '02', title: 'Website Conversion Improvements', text: 'Improve an existing website so visitors can understand the offer faster, trust the business and reach the next step more easily.', tags: ['UX', 'Conversion', 'Redesign'] },
  { n: '03', title: 'Quote & Lead Funnels', text: 'Focused landing pages, forms and enquiry flows for businesses that want more calls, quote requests or leads from their website.', tags: ['Landing Pages', 'Forms', 'Lead Flow'] },
...
const projects = [
  { title: 'Masonry / Construction Website', type: 'Business Website Concept', repo: 'pateljiop/Demo', href: 'https://pateljiop.github.io/Demo/', screenshot: 'https://image.thum.io/get/width/1400/crop/900/https://pateljiop.github.io/Demo/', summary: 'A service-business concept focused on trust, service clarity and a straightforward enquiry path.' },
  { title: 'Laundry Service Website', type: 'Local Business Concept', repo: 'pateljiop/Laundry-demo', href: 'https://pateljiop.github.io/Laundry-demo/', screenshot: 'https://image.thum.io/get/width/1400/crop/900/https://pateljiop.github.io/Laundry-demo/', summary: 'A local-service concept designed to make the offer easy to understand and the next action easy to find.' },
  { title: 'Roofing Website', type: 'Service Business Concept', repo: 'pateljiop/Roofing-demo', href: 'https://pateljiop.github.io/Roofing-demo/', screenshot: 'https://image.thum.io/get/width/1400/crop/900/https://pateljiop.github.io/Roofing-demo/', summary: 'A roofing-business concept built around credibility, service presentation and inspection enquiries.' },
  { title: 'Property Maintenance Website', type: 'Service Business Concept', repo: 'pateljiop/mek-property-maintenance-demo', href: 'https://pateljiop.github.io/mek-property-maintenance-demo/', screenshot: 'https://image.thum.io/get/width/1400/crop/900/https://pateljiop.github.io/mek-property-maintenance-demo/', summary: 'A property-maintenance concept showing a practical service catalogue and clear contact journey.' },
];
      ...
      <main>
        <section className="relative overflow-hidden border-b border-border/60">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-6 pb-24 pt-20 md:px-10 md:pb-32 md:pt-28 lg:grid-cols-[1.1fr_.9fr]">
            <div>
              ...
              <h1 className="mt-6 max-w-4xl text-5xl font-black leading-[.95] tracking-[-.04em] md:text-7xl lg:text-8xl">Websites built to turn visitors into <span className="text-primary">calls, quotes and enquiries.</span></h1>
              <p className="mt-7 max-w-2xl text-lg leading-8 text-muted-foreground md:text-xl">I build and improve websites for local businesses — with clear offers, stronger calls to action, mobile-first layouts and simple enquiry flows.</p>
              <div className="mt-9 flex flex-wrap gap-3">
                <a href="/#contact" className="rounded-xl bg-primary px-6 py-3.5 font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition hover:-translate-y-0.5 hover:opacity-90">Get a free website review →</a>
                <a href="#work" className="rounded-xl border border-border bg-background/60 px-6 py-3.5 font-semibold transition hover:border-primary/50 hover:bg-primary/5">See my work</a>
              </div>
              <div className="mt-9 flex flex-wrap gap-x-6 gap-y-2 font-mono text-xs text-muted-foreground"><span>Business Websites</span><span>Conversion</span><span>Lead Flows</span><span>Automation</span></div>
            </div>
        ...
        <section className="border-b border-border/60 bg-card/20">
          <div className="mx-auto grid max-w-7xl gap-8 px-6 py-14 md:grid-cols-3 md:px-10">
            {[['01', 'No fake client claims', 'Demo work is clearly labelled. I would rather show what I can build than invent a portfolio.'], ['02', 'Built for the business goal', 'The design follows the business problem, audience and conversion path — not a random template.'], ['03', 'Practical technology', 'I use code, APIs, Python and AI where they improve the result, not just to make the stack look impressive.']].map(([n, title, text]) => <div key={n} className="border-l border-primary/40 pl-5"><span className="font-mono text-xs text-primary">{n}</span><h2 className="mt-3 text-xl font-bold">{title}</h2><p className="mt-2 leading-7 text-muted-foreground">{text}</p></div>)}
          </div>
        </section>

        <section className="mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
            <div><p className="font-mono text-xs uppercase tracking-[.2em] text-primary">What I focus on</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">The website has a job to do.</h2></div>
            <div className="grid gap-3 sm:grid-cols-2">
              {[['Clearer messaging', 'Visitors should understand what you offer without hunting for the answer.'], ['Stronger CTAs', 'Make the next action obvious — call, request a quote, book or enquire.'], ['Mobile experience', 'Make it easy for people searching from their phone to take action.'], ['Trust', 'Present services, proof and important information professionally.']].map(([title, text], i) => <div key={title} className="rounded-2xl border border-border bg-card/30 p-5"><span className="font-mono text-xs text-primary">0{i + 1}</span><h3 className="mt-3 font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div>)}
            </div>
          </div>
        </section>

        <section id="services" className="mx-auto max-w-7xl px-6 pb-24 md:px-10 md:pb-32">
          <div className="max-w-3xl"><p className="font-mono text-xs uppercase tracking-[.2em] text-primary">Services</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">Choose the problem you need solved.</h2><p className="mt-5 text-lg leading-8 text-muted-foreground">Start with the business outcome. The technology comes after we understand what needs to improve.</p></div>
        ...
        <section id="work" className="border-y border-border/60 bg-card/20 py-24 md:py-32">
          <div className="mx-auto max-w-7xl px-6 md:px-10">
            ...
            <p className="mt-5 max-w-2xl leading-7 text-muted-foreground">These are demo/concept projects built to show the quality and direction of my work. They are not presented as paid client work.</p>
        ...
        <section id="contact" className="mx-auto max-w-7xl px-6 pb-28 md:px-10"><div className="relative overflow-hidden rounded-[2rem] border border-primary/25 bg-primary/5 p-8 text-center md:p-16"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,hsl(var(--primary)/.16),transparent_55%)]" /><div className="relative"><BrandMark compact /><p className="mt-6 font-mono text-xs uppercase tracking-[.2em] text-primary">Want to know what could be improved?</p><h2 className="mx-auto mt-4 max-w-3xl text-4xl font-black tracking-tight md:text-6xl">Get a free website review.</h2><p className="mx-auto mt-5 max-w-2xl leading-7 text-muted-foreground">Send me your website and I’ll look at the customer journey, clarity and enquiry flow, then point out one practical opportunity to improve it.</p><div className="mt-8 flex flex-wrap justify-center gap-3"><a href="/#contact" className="rounded-xl bg-primary px-7 py-3.5 font-semibold text-primary-foreground shadow-lg shadow-primary/20">Request a free review ↗</a><Link href="/#contact" className="rounded-xl border border-border bg-background/60 px-7 py-3.5 font-semibold">Back to portfolio</Link></div></div></div></section>
      </main>
      <Footer />
    </div>
  );
